...
pub async fn cleanup_all_apps<R: Runtime>(
    ...
    if versioned_count > 0 {
        ...
        log::info!("Running selective cleanup for {} regular packages", regular_packages.len());
...
pub async fn cleanup_all_apps_force(window: Window) -> Result<(), String> {
    ...
    run_cleanup_command(window, "scoop cleanup --all", "Force Cleanup All App Versions").await
}
