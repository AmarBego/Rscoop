...
async fn save_cache_to_disk(buckets: &HashMap<String, SearchableBucket>) -> Result<(), String> {
    ...
    log::info!("Saving {} buckets to cache file: {:?}", buckets.len(), cache_file);
...
async fn load_cache_from_disk() -> Result<HashMap<String, SearchableBucket>, String> {
    ...
    let csv_data = fs::read_to_string(&cache_file).await
    ...
    for result in reader.deserialize() {
        let bucket: SearchableBucket = result
            .map_err(|e| format!("Failed to deserialize bucket from CSV: {}", e))?;
...
pub async fn fetch_and_parse_bucket_directory(filters: Option<BucketFilterOptions>) -> Result<HashMap<String, SearchableBucket>, String> {
    ...
    log::info!("Downloaded {:.2} MB, parsing markdown table...", original_size_mb);
    ...
    log::info!("Applied filters: {} buckets filtered out, {} remaining (original: {})", 
               filtered_count, bucket_map.len(), original_count);
    ...
    log::info!("Original {:.2} MB markdown file processed and cleaned up from memory", original_size_mb);
...
fn parse_markdown_to_buckets(content: &str) -> Result<Vec<SearchableBucket>, String> {
    ...
    for (line_num, line) in content.lines().enumerate() {
        ...
        if let Some(captures) = complex_regex.captures(line) {
            ...
            let apps = captures.get(5).map_or("0", |m| m.as_str()).parse::<u32>().unwrap_or(0);
            let stars = captures.get(6).map_or("0", |m| m.as_str()).parse::<u32>().unwrap_or(0);
            let forks = captures.get(7).map_or("0", |m| m.as_str()).parse::<u32>().unwrap_or(0);
                ...
                if !bucket_name.is_empty() && !package_name.is_empty() {
                    ...
                    let (owner, repo_name, repo_url) = if package_url.contains("github.com") {
                        ...
                        if parts.len() >= 5 {
                            ...
                            ("unknown".to_string(), bucket_name.to_string(), package_url.to_string())
                        }
                    } else {
        ...
        if !parsed {
            if let Some(captures) = basic_regex.captures(line) {
                ...
                if !package_name.is_empty() {
                    ...
                    let (owner, repo_name, repo_url, bucket_name) = if package_url.contains("github.com") {
                        let parts: Vec<&str> = package_url.split('/').collect();
                        if parts.len() >= 5 {
                            let owner = parts[3];
                            let repo = parts[4];
                            (owner.to_string(), repo.to_string(), package_url.to_string(), repo.to_string())
                        } else {
                            ("unknown".to_string(), package_name.to_string(), package_url.to_string(), package_name.to_string())
                        }
                    } else {
                        ("unknown".to_string(), package_name.to_string(), package_url.to_string(), package_name.to_string())
                    };
...
fn parse_encoded_date(date_str: &str) -> String {
    ...
    if let Some(captures) = regex::Regex::new(r"^(\d{2})-(\d{1,2})-(\d{1,2})$").unwrap().captures(&cleaned) {
...
pub async fn get_cached_buckets(filters: Option<BucketFilterOptions>) -> Result<HashMap<String, SearchableBucket>, String> {
        ...
        Ok(disk_cache) if !disk_cache.is_empty() => {
            ...
            let filtered_cache = if let Some(ref filter_opts) = filters {
                if filter_opts.disable_chinese_buckets || filter_opts.minimum_stars > 0 {
                    ...
                    log::info!("Filtered cache: {} buckets filtered out, {} remaining (original: {})", 
                               filtered_count, filtered.len(), original_count);
                    filtered
                } else {
                    disk_cache
                }
